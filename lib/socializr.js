...
Socializr.prototype.stream = function(request, auths){
  ...
  for (var i = auths.length - 1; i >= 0; i--) {
    debug('Streaming data %j from %s with uuid %s',request,auths[i]._provider,auths[i]._provider.uuid);
...
Socializr.prototype.unstream = function(request, auths){
  ...
  var self = this;
  for (var i = auths.length - 1; i >= 0; i--) {
    debug('Unstreaming data %j from %s with uuid %s',request,auths[i]._provider,auths[i]._provider.uuid);
    auths[i]._provider.unstream( request, auths[i], this);
  };
};

Socializr.prototype._onError = function(err, conn){
  debug('Socializr captured an error %s with connection %s',err,conn);
